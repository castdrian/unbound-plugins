...
function resolveModule(source) {
	const candidate = resolve(pluginRoot, 'src', source.replace(/^@no-blocked-messages\//, ''));
...
function pluginAlias() {
	return {
		name: 'no-blocked-messages-alias',
		resolveId(source) {
			if (!source.startsWith('@no-blocked-messages/')) return null;
