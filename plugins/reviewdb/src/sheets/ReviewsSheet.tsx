...
function getDesignModule(): {
	...
	const discord = (metro as { components?: { Discord?: unknown } } | undefined)?.components
		?.Discord as { ActionSheet?: any } | undefined;
...
function ReviewsSheet({
	...
	const load = useCallback(
		async (offset: number, append: boolean) => {
			setLoading(true);
			const result = await getReviews(discordId, {
				offset,
	...
	const visibleReviews = (data?.reviews ?? []).filter(
		(review) => !hideBlockedUsers || !isRelationshipBlocked(review.sender.discordID),
	);
	...
	if (!Discord?.ActionSheet) {
		return (
			<ReactNative.View style={{ backgroundColor: colors.page, flex: 1, padding: 16 }}>
				<ReactNative.Text style={{ color: colors.text }}>
		...
		>
			<ReactNative.View style={{ gap: 8, paddingBottom: 4 }}>
				...
				<ReactNative.View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
					...
					>
						<ReactNative.Text style={{ color: colors.text, fontSize: 12, fontWeight: '700' }}>
							{data?.reviewCount ?? visibleReviews.length} reviews
						</ReactNative.Text>
					</ReactNative.View>
					{authorized && (
						<ReactNative.View
							style={{
								...
								borderColor: `${colors.positive}66`,
								borderRadius: 8,
								borderWidth: 1,
								paddingHorizontal: 9,
								paddingVertical: 5,
						...
						>
							<ReactNative.Text style={{ color: colors.positive, fontSize: 12, fontWeight: '700' }}>
								ReviewDB connected
							</ReactNative.Text>
						</ReactNative.View>
					)}
			...
			{loading && (
				...
				>
					...
					<ReactNative.Text style={{ color: colors.muted, fontSize: 14 }}>
						Loading reviews…
					</ReactNative.Text>
				</ReactNative.View>
			)}
			...
			{!loading && visibleReviews.length === 0 && (
				...
				>
					<ReactNative.Text style={{ color: colors.text, fontSize: 17, fontWeight: '800' }}>
						No reviews yet
					</ReactNative.Text>
			...
			{data?.hasNextPage && !loading && (
				...
				>
					<ReactNative.Text style={{ color: colors.link, fontSize: 14, fontWeight: '800' }}>
						Load more reviews
					</ReactNative.Text>
				</ReactNative.Pressable>
			)}
				...
				>
					<ReactNative.View style={{ gap: 4 }}>
						...
						<ReactNative.Text style={{ color: colors.muted, fontSize: 13, lineHeight: 18 }}>
							{ownReview
								? 'Your new text will replace your existing review.'
								: `Share helpful feedback about @${name}.`}
						</ReactNative.Text>
					</ReactNative.View>
					...
					<ReactNative.View style={{ alignItems: 'center', flexDirection: 'row', gap: 10 }}>
						<ReactNative.Text style={{ color: colors.muted, flex: 1, fontSize: 12 }}>
							{comment.length}/1000
						</ReactNative.Text>
						<ReactNative.Pressable
						...
						>
							{submitting ? (
								<ReactNative.ActivityIndicator color={colors.accentText} />
							) : (
								<ReactNative.Text
									style={{ color: colors.accentText, fontSize: 14, fontWeight: '800' }}
								>
			...
			{!isOwnProfile && !authorized && (
				...
				>
					<ReactNative.Text
						style={{ color: colors.link, fontSize: 15, fontWeight: '800', textAlign: 'center' }}
					>
