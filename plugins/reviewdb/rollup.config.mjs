...
function resolveReviewDBModule(source) {
	const candidate = resolve(pluginRoot, 'src', source.replace(/^@reviewdb\//, ''));
...
function reviewdbAlias() {
	return {
		name: 'reviewdb-alias',
		resolveId(source) {
			if (!source.startsWith('@reviewdb/')) return null;
			return resolveReviewDBModule(source);
		},
	};
}
