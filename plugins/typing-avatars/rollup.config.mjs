...
function resolveModule(source) {
	const candidate = resolve(pluginRoot, 'src', source.replace(/^@typing-avatars\//, ''));
	const search = [
		candidate,
		`${candidate}.ts`,
		`${candidate}.tsx`,
...
function pluginAlias() {
	return {
		name: 'typing-avatars-alias',
		resolveId(source) {
			if (!source.startsWith('@typing-avatars/')) return null;
...
export default {
	...
	plugins: [pluginAlias(), nodeResolve(), json(), swc({ tsconfig: false }), iife(), hermesExpressionEntrypoint(), manifestToDist()],
	output: {
		dir: 'dist',
